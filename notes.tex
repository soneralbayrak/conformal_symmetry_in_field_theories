...
\be 
\left(1+a_kp_k+\frac{(a_kp_k)^2}{2}+\dots\right)f(x)=f(x)+a_k\partial_kf(x)+\frac{a_k a_l}{2}\partial_k\partial_lf(x)+\dots
\ee 
Clearly, we can simply choose $p_k=\partial_k$, which satisfies the necessary algebra condition in \equref{eq: translation algebra}. However, we will derive this with a somehow complicated method, because the approach will generalize to other groups as well.

Let's consider the \emph{Fourier representation} of $f(x)$, i.e.
\be 
f(x)=\int\limits_{-\infty}^\infty\frac{d^dq}{(2\pi)^d}e^{iq\.x}\tilde f(q)
\ee 
...
\be 
f(x+a)=&\int\limits_{-\infty}^\infty\frac{d^dq}{(2\pi)^d}e^{iq\.(x+a)}\tilde f(q)=\int\limits_{-\infty}^\infty\frac{d^dq}{(2\pi)^d}e^{iq\.a}e^{iq\.x}\tilde f(q)=\int\limits_{-\infty}^\infty\frac{d^dq}{(2\pi)^d}e^{a\.\partial}e^{iq\.x}\tilde f(q)
\\&=e^{a\.\partial}\int\limits_{-\infty}^\infty\frac{d^dq}{(2\pi)^d}e^{iq\.x}\tilde f(q)=e^{q\.\partial}f(x)
\ee 
...
\comm{m_{ab}}{m_{cd}}=\eta_{ad}m_{bc}+\eta_{bc}m_{da}+\eta_{ac}m_{db}+\eta_{bd}m_{ac}
\ee 
...
\be 
f(r,\theta,\phi)=\sum\limits_{l=0}^\infty \sum\limits_{m=-l}^l\left[r^lP_l^m(\cos\theta)e^{im\f}\right]\tilde{f}(l,m)
\ee 
...
\be 
f(r,\theta,\phi+a)=&f(r,\theta,\phi)=\sum\limits_{l=0}^\infty \sum\limits_{m=-l}^l\left[r^lP_l^m(\cos\theta)e^{im(\f+a)}\right]\tilde{f}(l,m)
\\=&f(r,\theta,\phi)=\sum\limits_{l=0}^\infty \sum\limits_{m=-l}^le^{a\partial_\f}\left[r^lP_l^m(\cos\theta)e^{im\f}\right]\tilde{f}(l,m)
...
\be 
\frac{\partial}{\partial \f}=\frac{\partial x}{\partial \f}\frac{\partial}{\partial x}+\frac{\partial y}{\partial \f}\frac{\partial}{\partial y}+\frac{\partial z}{\partial \f}\frac{\partial}{\partial z}=x\partial_y-y\partial_x
\ee 
...
Even though we showed for $d=3$, the situation generalizes: $m_{ab}=x_a\partial_b-x_b\partial_a$ generate the $so(d)$ algebra and one can check that the commutation rule in \equref{eq: rotation algebra} is satisfied!
...
\be 
f(r)=\int\limits_{c-i\infty}^{{c+i\infty}}\frac{ds}{2\pi i}r^{-s}\tl f(s)
\ee 
...
\be 
f(\lambda r)=&\int\limits_{c-i\infty}^{{c+i\infty}}\frac{ds}{2\pi i}(e^ r)^{-s}\tl f(s)
=\int\limits_{c-i\infty}^{{c+i\infty}}\frac{ds}{2\pi i} e^{-\lambda s} r^{-s}\tl f(s)
=\int\limits_{c-i\infty}^{{c+i\infty}}\frac{ds}{2\pi i} e^{\lambda \partial/\partial(\log r)} r^{-s}\tl f(s)
\\&=e^{\lambda \partial/\partial(\log r)}\int\limits_{c-i\infty}^{{c+i\infty}}\frac{ds}{2\pi i}  r^{-s}\tl f(s)
